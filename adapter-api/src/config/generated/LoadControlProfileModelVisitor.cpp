                    ...
                    for(auto count4 = 0; count4 < max_count4; ++count4)
                    {
                        ...
                        visitor.handle(
                            ...
                            PrimitiveAccessorBuilder<commonmodule::StateKind, loadmodule::LoadControlProfile>::build(
                                [context = mutable_context5](loadmodule::LoadControlProfile& profile, commonmodule::StateKind value) { return context(profile)->set_state(value); },
                                [context = const_context5](const loadmodule::LoadControlProfile& profile) { const auto temp = context(profile); return temp ? temp->state() : throw Exception("Primitive value parent(s) not present: loadControl.loadControlFSCC.loadControlScheduleFSCH.ValDCSG.crvPts.state "); },
                                [context = const_context5](const loadmodule::LoadControlProfile& profile) -> bool { return context(profile) != nullptr; }
                            )
                        );
                        visitor.end_iteration();
                    }
