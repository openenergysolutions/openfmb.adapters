...
void convert_to_proto(const twinoaks::switchmodule::SwitchControlScheduleFSCH& in, switchmodule::SwitchControlScheduleFSCH& out);
...
void convert_to_proto(const twinoaks::switchmodule::SwitchPoint& in, switchmodule::SwitchPoint& out);
...
void convert_to_proto(const twinoaks::switchmodule::SwitchCSG& in, switchmodule::SwitchCSG& out);
...
void convert_to_proto(const twinoaks::commonmodule::ScheduleCSG& in, commonmodule::ScheduleCSG& out)
{
    ...
    if(in.xD) out.mutable_xd()->set_value(in.xD);

    if(in.xDU) out.mutable_xdu()->set_value(in.xDU);

    if(in.yD) out.mutable_yd()->set_value(in.yD);

    if(in.yDU) out.mutable_ydu()->set_value(in.yDU);

    convert_to_proto(in.yUnits, *out.mutable_yunits()); // required field in DDS

    if(in.zD) out.mutable_zd()->set_value(in.zD);

    if(in.zDU) out.mutable_zdu()->set_value(in.zDU);

    if(in.zUnits) convert_to_proto(*in.zUnits, *out.mutable_zunits());
}
...
//    out.set_name(static_cast<essmodule::ESSFunctionParameterKind>(in.name));
...
void convert_to_proto(const twinoaks::commonmodule::ControlFSCC& in, commonmodule::ControlFSCC& out)
{
    ...
    for(decltype(in.controlScheduleFSCH.length()) i = 0; i < in.controlScheduleFSCH.length(); ++i)
    {
        convert_to_proto(in.controlScheduleFSCH.at(i), *out.mutable_controlschedulefsch()->Add());
    }
}
...
void convert_to_proto(const twinoaks::commonmodule::SchedulePoint& in, commonmodule::SchedulePoint& out)
{
    ...
    out.set_yval(in.yVal);

    if(in.zVal) out.mutable_zval()->set_value(*in.zVal);
}
