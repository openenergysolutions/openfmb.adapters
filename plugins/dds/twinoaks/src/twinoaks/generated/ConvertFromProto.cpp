...
void convert_from_proto(const commonmodule::Timestamp& in, twinoaks::commonmodule::Timestamp& out)
{
    ...
    if(in.has_tq()) out.tq = allocate_from_proto<twinoaks::commonmodule::TimeQuality>(in.tq());
...
void convert_from_proto(const commonmodule::WYE& in, twinoaks::commonmodule::WYE& out)
{
    out.clear();
    // convert message fields
    if(in.has_net()) out.net = allocate_from_proto<twinoaks::commonmodule::CMV>(in.net());
    if(in.has_neut()) out.neut = allocate_from_proto<twinoaks::commonmodule::CMV>(in.neut());
    if(in.has_phsa()) out.phsA = allocate_from_proto<twinoaks::commonmodule::CMV>(in.phsa());
    if(in.has_phsb()) out.phsB = allocate_from_proto<twinoaks::commonmodule::CMV>(in.phsb());
    if(in.has_phsc()) out.phsC = allocate_from_proto<twinoaks::commonmodule::CMV>(in.phsc());
}
...
void convert_from_proto(const commonmodule::CMV& in, twinoaks::commonmodule::CMV& out)
{
    ...
    if(in.has_units()) out.units = allocate_from_proto<twinoaks::commonmodule::Unit>(in.units());
}
...
void convert_from_proto(const commonmodule::ACDCTerminal& in, twinoaks::commonmodule::ACDCTerminal& out)
{
    ...
    if(in.has_connected()) out.connected = new unsigned char(static_cast<unsigned char>(in.connected().value()));
    if(in.has_sequencenumber()) out.sequenceNumber = new int(in.sequencenumber().value());
}
...
void convert_from_proto(const commonmodule::IdentifiedObject& in, twinoaks::commonmodule::IdentifiedObject& out)
{
    out.clear();
    // convert message fields
    if(in.has_description()) out.description = allocate_string(in.description().value());
    if(in.has_mrid()) out.mRID = allocate_string(in.mrid().value());
    if(in.has_name()) out.name = allocate_string(in.name().value());
}
...
void convert_from_proto(const resourcemodule::ResourceReading& in, twinoaks::resourcemodule::ResourceReading& out)
{
    ...
    if(in.has_phasemmtn()) out.phaseMMTN = allocate_from_proto<twinoaks::commonmodule::PhaseMMTN>(in.phasemmtn());
    if(in.has_readingmmtr()) out.readingMMTR = allocate_from_proto<twinoaks::commonmodule::ReadingMMTR>(in.readingmmtr());
    if(in.has_readingmmxu()) out.readingMMXU = allocate_from_proto<twinoaks::commonmodule::ReadingMMXU>(in.readingmmxu());
}
...
void convert_from_proto(const commonmodule::ReadingMMTR& in, twinoaks::commonmodule::ReadingMMTR& out)
{
    ...
    if(in.has_dmdvah()) out.DmdVAh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.dmdvah());
    if(in.has_dmdvarh()) out.DmdVArh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.dmdvarh());
    if(in.has_dmdwh()) out.DmdWh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.dmdwh());
    if(in.has_supvah()) out.SupVAh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.supvah());
    if(in.has_supvarh()) out.SupVArh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.supvarh());
    if(in.has_supwh()) out.SupWh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.supwh());
    if(in.has_totvah()) out.TotVAh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.totvah());
    if(in.has_totvarh()) out.TotVArh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.totvarh());
    if(in.has_totwh()) out.TotWh = allocate_from_proto<twinoaks::commonmodule::BCR>(in.totwh());
}
...
void convert_from_proto(const breakermodule::BreakerReading& in, twinoaks::breakermodule::BreakerReading& out)
{
    ...
    // convert message fields
    if(in.has_conductingequipmentterminalreading()) convert_from_proto(in.conductingequipmentterminalreading(), out); // inherited type
    if(in.has_diffreadingmmxu()) out.diffReadingMMXU = allocate_from_proto<twinoaks::commonmodule::ReadingMMXU>(in.diffreadingmmxu());
    if(in.has_phasemmtn()) out.phaseMMTN = allocate_from_proto<twinoaks::commonmodule::PhaseMMTN>(in.phasemmtn());
    if(in.has_readingmmtr()) out.readingMMTR = allocate_from_proto<twinoaks::commonmodule::ReadingMMTR>(in.readingmmtr());
    if(in.has_readingmmxu()) out.readingMMXU = allocate_from_proto<twinoaks::commonmodule::ReadingMMXU>(in.readingmmxu());
}
...
void convert_from_proto(const commonmodule::PhaseMMTN& in, twinoaks::commonmodule::PhaseMMTN& out)
{
    out.clear();
    // convert message fields
    if(in.has_phsa()) out.phsA = allocate_from_proto<twinoaks::commonmodule::ReadingMMTN>(in.phsa());
    if(in.has_phsab()) out.phsAB = allocate_from_proto<twinoaks::commonmodule::ReadingMMTN>(in.phsab());
    if(in.has_phsb()) out.phsB = allocate_from_proto<twinoaks::commonmodule::ReadingMMTN>(in.phsb());
    if(in.has_phsbc()) out.phsBC = allocate_from_proto<twinoaks::commonmodule::ReadingMMTN>(in.phsbc());
    if(in.has_phsc()) out.phsC = allocate_from_proto<twinoaks::commonmodule::ReadingMMTN>(in.phsc());
    if(in.has_phsca()) out.phsCA = allocate_from_proto<twinoaks::commonmodule::ReadingMMTN>(in.phsca());
}
...
void convert_from_proto(const commonmodule::ReadingMMXU& in, twinoaks::commonmodule::ReadingMMXU& out)
{
    ...
    // convert message fields
    if(in.has_logicalnode()) convert_from_proto(in.logicalnode(), out); // inherited type
    if(in.has_a()) out.A = allocate_from_proto<twinoaks::commonmodule::WYE>(in.a());
    if(in.has_clcmth()) out.ClcMth = allocate_from_proto<twinoaks::commonmodule::ENG_CalcMethodKind>(in.clcmth());
    if(in.has_hz()) out.Hz = allocate_from_proto<twinoaks::commonmodule::MV>(in.hz());
    if(in.has_pf()) out.PF = allocate_from_proto<twinoaks::commonmodule::WYE>(in.pf());
    if(in.has_pfsign()) out.PFSign = allocate_from_proto<twinoaks::commonmodule::ENG_PFSignKind>(in.pfsign());
    if(in.has_phv()) out.PhV = allocate_from_proto<twinoaks::commonmodule::WYE>(in.phv());
    if(in.has_ppv()) out.PPV = allocate_from_proto<twinoaks::commonmodule::DEL>(in.ppv());
    if(in.has_va()) out.VA = allocate_from_proto<twinoaks::commonmodule::WYE>(in.va());
    if(in.has_var()) out.VAr = allocate_from_proto<twinoaks::commonmodule::WYE>(in.var());
    if(in.has_w()) out.W = allocate_from_proto<twinoaks::commonmodule::WYE>(in.w());
}
...
void convert_from_proto(const commonmodule::AnalogueValue& in, twinoaks::commonmodule::AnalogueValue& out)
{
    out.clear();
    // convert message fields
    if(in.has_f()) out.f = new float(in.f().value());
    if(in.has_i()) out.i = new int(in.i().value());
}
...
void convert_from_proto(const commonmodule::Vector& in, twinoaks::commonmodule::Vector& out)
{
    out.clear();
    // convert message fields
    if(in.has_ang()) out.ang = allocate_from_proto<twinoaks::commonmodule::AnalogueValue>(in.ang());
...
void convert_from_proto(const commonmodule::DEL& in, twinoaks::commonmodule::DEL& out)
{
    out.clear();
    // convert message fields
    if(in.has_phsab()) out.phsAB = allocate_from_proto<twinoaks::commonmodule::CMV>(in.phsab());
    if(in.has_phsbc()) out.phsBC = allocate_from_proto<twinoaks::commonmodule::CMV>(in.phsbc());
    if(in.has_phsca()) out.phsCA = allocate_from_proto<twinoaks::commonmodule::CMV>(in.phsca());
}
