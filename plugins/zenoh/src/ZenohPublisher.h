...
#ifndef OPENFMB_ADAPTER_ZENOH_PUBLISHER_H
#define OPENFMB_ADAPTER_ZENOH_PUBLISHER_H
...
namespace zenoh {
    template <class T>
    class ZenohPublisher final : public api::ISubscriptionHandler<T> // a Zenoh publisher subscribes to the internal bus
    {

    public:
        using message_queue_t = util::SynchronizedQueue<util::Message>;

        ZenohPublisher(api::Logger logger, const util::SubjectNameSuffix& suffix, std::shared_ptr<message_queue_t> sink)
...
#endif //OPENFMB_ADAPTER_ZENOH_PUBLISHER_H
