    ...
    private:
        ...
        void configure_float(const YAML::Node& node, const std::function<void(T&, float)>& setter)
        {
            ...
            case (MappingType::uint16):
                this->map_register16(node, [setter, scale = get_float_scale(node)](T& profile, const std::shared_ptr<Register16>& reg, Logger&) {
            ...
            case (MappingType::uint32):
                this->map_register32(node, [setter, scale = get_float_scale(node)](T& profile, const std::shared_ptr<Register32>& reg, Logger&) {
            ...
            case (MappingType::uint32_with_modulus):
                this->map_register32(node, [setter, scale = get_float_scale(node), modulus = get_modulus(node)](T& profile, const std::shared_ptr<Register32>& reg, Logger&) {
