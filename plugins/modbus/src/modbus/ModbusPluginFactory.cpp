...
#include "ModbusConfigWriteVisitor.h"
...
namespace adapter
{
    ...
    void ModbusPluginFactory::write_default_session(const std::string& name, YAML::Emitter& out)
    {
        out << YAML::BeginMap;
        ...
        out << YAML::BeginMap;
        ModbusConfigWriteVisitor<resourcemodule::ResourceReadingProfile> visitor(out);
        visit(visitor);
        out << YAML::EndMap;
    ...
    void ModbusPluginFactory::write_default_poll(YAML::Emitter& out, uint16_t start, uint16_t count)
    {
        out << YAML::BeginMap;
