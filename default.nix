...
pkgs.callPackage ({ninja, cmake, protobuf, openssl, libpcap, boost166, asio, spdlog, catch2, deps }: pkgs.stdenv.mkDerivation {
  ...
  cmakeFlags = [ "-DOPENFMB_USE_CONAN=OFF" "-DOPENFMB_USE_GOOSE=OFF" "-DOPENFMB_USE_ZENOH=OFF" "-DOPENFMB_USE_CAPTURE=ON" "-DOPENFMB_USE_REPLAY=ON" "-DCMAKE_BUILD_TYPE=Release"];
