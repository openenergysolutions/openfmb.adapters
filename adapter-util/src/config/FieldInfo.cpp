...
namespace util {
    namespace fields {
        ...
        EnumFieldType::Value get_enum_type(google::protobuf::EnumDescriptor const* descriptor)
        {
            ...
            if (elem == map.end()) {
                throw api::Exception("Unknown type for enum: ", descriptor->full_name());
            }
            return elem->second;
        }
        ...
        E find_mapping_or_throw(const field_map_t<E>& map, const std::string& field_name, IDescriptorPath& path)
        {
            ...
            if (elem == map.end())
                throw api::Exception("Unknown field name: ", path.as_string(), ".", field_name);
            ...
            if (match == elem->second.end())
                throw api::Exception("No mapping for field: ", path.as_string(), ".", field_name);
