...
void visit_commonmodule_AnalogStatusGGIO(const set_t<commonmodule::AnalogStatusGGIO>& setter, const get_t<commonmodule::AnalogStatusGGIO>& getter, ITypedModelVisitor<resourcemodule::ResourceStatusProfile>& visitor);
...
void visit_commonmodule_BooleanStatusGGIO(const set_t<commonmodule::BooleanStatusGGIO>& setter, const get_t<commonmodule::BooleanStatusGGIO>& getter, ITypedModelVisitor<resourcemodule::ResourceStatusProfile>& visitor);
...
void visit_commonmodule_IntegerStatusGGIO(const set_t<commonmodule::IntegerStatusGGIO>& setter, const get_t<commonmodule::IntegerStatusGGIO>& getter, ITypedModelVisitor<resourcemodule::ResourceStatusProfile>& visitor);
...
void visit_resourcemodule_ResourceStatus(const set_t<resourcemodule::ResourceStatus>& setter, const get_t<resourcemodule::ResourceStatus>& getter, ITypedModelVisitor<resourcemodule::ResourceStatusProfile>& visitor)
{
    if(visitor.start_message_field("statusValue", commonmodule::StatusValue::descriptor()))
    {
        visit_commonmodule_StatusValue(
            ...
            [getter](const resourcemodule::ResourceStatusProfile& profile) -> commonmodule::StatusValue const *
            {
                const auto value = getter(profile);
                if(value)
                {
    ...
    {
        const auto count = visitor.start_repeated_message_field("analogstatusggio", commonmodule::AnalogStatusGGIO::descriptor());
        for(int i = 0; i < count; ++i)
        {
            visitor.start_iteration(i);
            const auto set = [setter, i, max = count](resourcemodule::ResourceStatusProfile& profile)
            {
                const auto repeated = setter(profile)->mutable_analogstatusggio();
            ...
            const auto get = [getter, i](const resourcemodule::ResourceStatusProfile& profile) -> commonmodule::AnalogStatusGGIO const*
            {
                const auto value = getter(profile);
                if(value)
                {
                    return (i < value->analogstatusggio_size()) ? &value->analogstatusggio(i) : nullptr;
                }
                else
                {
                    return nullptr;
            ...
            visit_commonmodule_AnalogStatusGGIO(set, get, visitor);
    ...
    {
        const auto count = visitor.start_repeated_message_field("booleanstatusggio", commonmodule::BooleanStatusGGIO::descriptor());
        for(int i = 0; i < count; ++i)
        {
            visitor.start_iteration(i);
            const auto set = [setter, i, max = count](resourcemodule::ResourceStatusProfile& profile)
            {
                const auto repeated = setter(profile)->mutable_booleanstatusggio();
                if(repeated->size() < max)
                {
                    repeated->Reserve(max);
                    // add items until we're at max requested capacity
            ...
            const auto get = [getter, i](const resourcemodule::ResourceStatusProfile& profile) -> commonmodule::BooleanStatusGGIO const*
            {
                const auto value = getter(profile);
                if(value)
                {
                    return (i < value->booleanstatusggio_size()) ? &value->booleanstatusggio(i) : nullptr;
                }
                else
                {
                    return nullptr;
            ...
            visit_commonmodule_BooleanStatusGGIO(set, get, visitor);
            visitor.end_iteration();
        }
        visitor.end_repeated_message_field();
    }
    ...
    {
        const auto count = visitor.start_repeated_message_field("integerstatusggio", commonmodule::IntegerStatusGGIO::descriptor());
        for(int i = 0; i < count; ++i)
        {
            visitor.start_iteration(i);
            const auto set = [setter, i, max = count](resourcemodule::ResourceStatusProfile& profile)
            {
                const auto repeated = setter(profile)->mutable_integerstatusggio();
                if(repeated->size() < max)
                {
                    repeated->Reserve(max);
                    // add items until we're at max requested capacity
            ...
            const auto get = [getter, i](const resourcemodule::ResourceStatusProfile& profile) -> commonmodule::IntegerStatusGGIO const*
            {
                const auto value = getter(profile);
                if(value)
                {
                    return (i < value->integerstatusggio_size()) ? &value->integerstatusggio(i) : nullptr;
                }
                else
                {
                    return nullptr;
            ...
            visit_commonmodule_IntegerStatusGGIO(set, get, visitor);
            visitor.end_iteration();
        }
        visitor.end_repeated_message_field();
    }
    ...
    {
        const auto count = visitor.start_repeated_message_field("stringstatusggio", commonmodule::StringStatusGGIO::descriptor());
        for(int i = 0; i < count; ++i)
        {
            visitor.start_iteration(i);
            const auto set = [setter, i, max = count](resourcemodule::ResourceStatusProfile& profile)
            {
                const auto repeated = setter(profile)->mutable_stringstatusggio();
                if(repeated->size() < max)
                {
                    repeated->Reserve(max);
                    // add items until we're at max requested capacity
            ...
            const auto get = [getter, i](const resourcemodule::ResourceStatusProfile& profile) -> commonmodule::StringStatusGGIO const*
            {
                const auto value = getter(profile);
                if(value)
                {
                    return (i < value->stringstatusggio_size()) ? &value->stringstatusggio(i) : nullptr;
                }
                else
                {
                    return nullptr;
            ...
            visit_commonmodule_StringStatusGGIO(set, get, visitor);
            visitor.end_iteration();
        }
        visitor.end_repeated_message_field();
    }
